...
// === drawer.ts – Notification helpers ===
...
function showToastDrawer(text: string, timeout = 4000) {
...
function broadcastSelection(eventType: "selectShape" | "unselectShape", shapeId: number) {
...
export function connectWebSocket(canvasId: string, shapeManager: ShapeManager, canvas: Canvas) {
  ...
  //window.dispatchEvent(new CustomEvent("websocket-ready"));
    ...
    replayFromText(msg.log, shapeManager, canvas);
        ...
        if (msg.clientId === globalClientId) return; // eigene Nachricht ignorieren
    ...
    /*else if (msg.event === "accessChanged" && msg.canvasId === canvasId) {
        ...
        const checkbox = document.getElementById("toggleModerated") as HTMLInputElement | null;
        ...
        if (msg.targetUserId === globalUserId) {
            ...
            fetch(`/api/is-moderated?canvasId=${canvasId}`)
                .then(r => r.json())
                .then(d => {
                const cb = document.getElementById("toggleModerated") as HTMLInputElement | null;
                if (cb) cb.checked = !!d.isModerated;
                })
                .catch(console.error);

            // Liste der Online-Benutzer anfordern
            (window as any)._activeWS?.send(JSON.stringify({
    ...
    // 2) Zugriffsrecht dieses Benutzers geändert (gilt in jedem Tab)
        ...
        if (msg.canvasId === globalCanvasId) {
            ...
            if (myAccessLevel === "R") {
                ...
                if (isModerated) disableToolsUI(); else enableToolsUI();
            } 
            else {
                enableToolsUI();
            }
...
export function cleanupCanvas(currentCanvasId: string) {
  if (globalSocket && globalSocket.readyState === WebSocket.OPEN) {
    globalSocket.send(JSON.stringify({
...
function appendEventLog(message: string, forReplay: boolean = false, sendToServer: boolean = false) {
    const logElement = document.getElementById('eventLog') as HTMLTextAreaElement;
    if (logElement) {
...
interface Shape {
    ...
    fillColor: string;
    borderColor: string;
    draw(ctx: CanvasRenderingContext2D, marked?: boolean): void;
    getExportData(): any;
}
class Point2D {
...
class Line extends AbstractShape implements Shape {
    ...
    constructor(from: Point2D, to: Point2D, id?: number){
        super(id);
        this.from = from;
        this.to = to;
    }
    draw(ctx: CanvasRenderingContext2D, marked: boolean = false) {
...
class Circle extends AbstractShape implements Shape {
    ...
    constructor(center: Point2D, radius: number, id?: number){
        super(id);
        this.center = center;
        this.radius = radius;
    }
...
class Triangle extends AbstractShape implements Shape {
    ...
    constructor(p1: Point2D, p2: Point2D, p3: Point2D, id?: number){
        super(id);
        this.p1 = p1;
        this.p2 = p2;
        this.p3 = p3;
...
class ToolArea {
    ...
    setShapeFactories(factories: ShapeFactory[], menue: Element) {
...
class Canvas implements ShapeManager {
    ...
    insertShapeAt(shape: Shape, index: number, redraw: boolean = true): this {
    ...
    addShape(shape: Shape, redraw: boolean = true, temporary: boolean = false): this {
    ...
    if (idx !== -1) this.shapeList.splice(idx, 1);
    return redraw ? this.draw() : this;
    }
...
class SelectionFactory implements ShapeFactory {
    private dragStart: Point2D | null = null;

    public label = "Selektion";

    private lastHits: Shape[] = [];

    private lastHitIndex: number = 0;

    constructor(private getCanvas: () => Canvas) {}
    ...
    handleMouseMove(x: number, y: number): void {
        ...
        canvas.selectedIds.forEach(id => {
            const shape = (canvas as any).shapes[id];
            if (shape instanceof Line) {
...
handleMouseUp(x: number, y: number): void {
    ...
    canvas.selectedIds.forEach(id => {
        const original = (canvas as any).shapes[id];
        ...
        if (original instanceof Line) {
            newShape = new Line(
                new Point2D(original.from.x + dx, original.from.y + dy),
                new Point2D(original.to.x + dx, original.to.y + dy),
                original.id
            );
        } else if (original instanceof Circle) {
            newShape = new Circle(
                new Point2D(original.center.x + dx, original.center.y + dy),
                original.radius,
                original.id
            );
            ...
            newShape = new Triangle(
                new Point2D(original.p1.x + dx, original.p1.y + dy),
                new Point2D(original.p2.x + dx, original.p2.y + dy),
                new Point2D(original.p3.x + dx, original.p3.y + dy),
                original.id
            );
        }
        ...
        if (newShape) {
            ...
            // 6. Event senden
            appendEventLog(`DRAW ${newShape.constructor.name} ${JSON.stringify({
                ...newShape,
                zOrder: index // Position mitloggen
            })}`, true, true);
        }
    });
...
const COLOR_OPTIONS: {[css: string]: string} = {
...
const BORDER_COLOR_OPTIONS = Object.fromEntries(
  Object.entries(COLOR_OPTIONS).filter(([css]) => css !== "rgba(0,0,0,0)")
);
...
export function initCanvas(canvasId: string) {
    ...
    const menu = document.getElementsByClassName("tools")[0] as HTMLElement;
    globalCanvasId = canvasId;
    ...
    fetch("/api/me")
        .then(res => res.json())
        .then(user => {
        const info = user.canvases.find((c: any) => c.canvasId === canvasId);
    ...
    const acc = me.canvases.find((c: any) => c.canvasId === canvasId);
    myAccessLevel = acc?.right ?? null;
    ...
    canvasDomElm.addEventListener("contextmenu", (e: MouseEvent) => {
        e.preventDefault();
...
contextMenu.createRadioOption(
  "Hintergrundfarbe",
  COLOR_OPTIONS,
  (() => {
    const ids = Array.from(canvas.selectedIds);
    return ids.length
      ? (canvas as any).shapes[ids[0]].fillColor
      : null;
  })(),
  (cssKey: string) => {
    canvas.selectedIds.forEach(id => {
...
contextMenu.createRadioOption(
  ...
  (() => {
    const ids = Array.from(canvas.selectedIds);
    return ids.length
      ? (canvas as any).shapes[ids[0]].borderColor
      : null;
  })(),
  (cssKey: string) => {
    canvas.selectedIds.forEach(id => {
...
export function replayFromText(singleLine: string, shapeManager: ShapeManager, canvasInstance: Canvas): void {
    const shapeMap = new Map<number, Shape>(); // nur ein Event, daher kein Reset

    if (singleLine.startsWith("DRAW")) {
        const [_, type, ...data] = singleLine.split(" ");
        const shapeData = JSON.parse(data.join(" "));
        const id = shapeData._id ?? shapeData.id;
        let shape: Shape | undefined;
    ...
    if (shape) {
        ...
        const zOrder = shapeData.zOrder ?? null;
